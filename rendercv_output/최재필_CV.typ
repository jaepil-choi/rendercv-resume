...
#one-col-entry(
  content: [
    #v(design-highlights-top-margin);#highlights([내부 딜러들이 코딩 없이 글로벌 매크로\(채권\/주식인덱스\/원자재\/FX\) 전략을 엑셀 함수 입력하듯 만들 수 있는 플랫폼 개발 중],[다양한 객체지향 디자인패턴을 도입하여 전략\/유틸 기능 추가가 용이하고 데이터 쿼리, 전\/후처리 등이 설정파일\(config\) 조작만으로 적용\/수정될 수 있도록 확장성을 고민해 설계],)
  ],
)
